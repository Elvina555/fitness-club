...
<head>
  ...
  <title>Тест API: Абонементы</title>
  <style>
    body {
...
<body>
  <div class="container">
    <h1>ТЕСТ API: CLIENT-SUBSCRIPTIONS</h1>

    <div class="url">URL: api/client/client-subscriptions.php</div>
  ...
  <script>
          ...
          try {
            ...
            if (jsonData.success) {
              ...
              if (jsonData.subscriptions) {
                statusEl.innerHTML += ' (абонементов: ' + jsonData.subscriptions.length + ')';
              }
    ...
    function formatJSON(data) {
      ...
      if (data.subscriptions && Array.isArray(data.subscriptions)) {
        html += '<div>subscriptions: ' + data.subscriptions.length + ' записей</div>';

        let table = '<table>';
        table += '<tr><th>ID</th><th>Тип</th><th>Дата начала</th><th>Дата конца</th><th>Статус</th><th>Посещения</th></tr>';

        data.subscriptions.forEach(sub => {
          table += '<tr>';
          table += '<td>' + (sub.id || '') + '</td>';
          table += '<td>' + (sub.type || '') + '</td>';
          table += '<td>' + (sub.start_date || '') + '</td>';
          table += '<td>' + (sub.end_date || '') + '</td>';
          table += '<td>' + (sub.status || '') + '</td>';
          table += '<td>' + (sub.visits_left || 0) + '/' + (sub.visits_total || 0) + '</td>';
          table += '</tr>';
        });
